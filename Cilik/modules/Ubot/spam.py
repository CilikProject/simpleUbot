...
async def statspam(client: Client, message: Message):
    yanto = await message.reply_text("⚡ Usage:\n /delspam 10 Cilik")
...
async def delayspam(client, message):
    ...
    pesan = str(userbot[1])
    kk = await message.edit(f"Starting delayspam number of messages {jumlah} with time {waktu}")
...
async def spam_stick(client: Client, message: Message):
    if not message.reply_to_message:
        await message.edit_text("reply to a sticker")
        return
